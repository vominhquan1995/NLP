...
class ClassificationPow:
    ...
    def run(self):
        ...
        path_output ="output/log_{}_running_{}.txt".format(self.option, datetime.today().strftime("%H%M%S%d%m%Y")) 
        ...
        f_output.writelines("------------------------------------------------\n")
        f_output.writelines('Avg precision is %s \n' % ClassificationPow.Average(precision_avg))
        f_output.writelines('Avg recall is %s \n' % ClassificationPow.Average(recall_avg))
        f_output.writelines('Avg f1 is %s \n' % ClassificationPow.Average(f1_avg))
